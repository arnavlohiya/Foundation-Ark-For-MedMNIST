...
def ark_engine(args, model_path, output_path, dataset_list, datasets_config, dataset_train_list, dataset_val_list, dataset_test_list):
    ...
    # ===================================================================
    # MEDMNIST MODIFICATION: Multiple Loss Functions Support
    # Different task types require different loss functions
    # ===================================================================
    
    def get_loss_function(task_type):
        """
        Return appropriate loss function based on MedMNIST task type
        
        Args:
            task_type: String indicating the type of task
        ...
        if task_type == "multi-class classification":
            ...
            # For ordinal regression, could use MSE or specialized ordinal loss
            # Using CrossEntropy for now as it works well in practice
            return torch.nn.CrossEntropyLoss()
        else:
            # Default to BCEWithLogitsLoss for backwards compatibility
            return torch.nn.BCEWithLogitsLoss()
    
    # Create dataset-specific loss functions for multi-task learning
    loss_functions = []
    for dataset in dataset_list:
        task_type = datasets_config[dataset]['task_type']
        loss_fn = get_loss_function(task_type)
        loss_functions.append(loss_fn)
        print(f"Dataset {dataset}: Using {task_type} with {type(loss_fn).__name__}")
    
    # Keep original criterion for backwards compatibility with evaluate function
    ...
    if args.from_checkpoint:
        ...
        model = build_omni_model(args, num_classes_list)#this is the student model
        teacher = build_omni_model(args, num_classes_list) #teacher model 
    print(model)

    if torch.cuda.device_count() > 1:#How many GPUs are accessible.
        model = torch.nn.DataParallel(model)
        teacher = torch.nn.DataParallel(teacher)#splits each batch across different GPUs, for parallel execution.
        """Why this matters:
        - GPU memory and CPU memory are physically separate
        - Data transfer between them is expensive
        - PyTorch requires explicit device placement for performance and clarity
        """
    model.to(device)#Transfer all model parameters and buffers from CPU memory to the target device i.e cuda (for GPU)
    teacher.to(device)#models must be on the same device as the data.
    for p in teacher.parameters():
        p.requires_grad = False
    print(f"Student and Teacher are built: they are both {args.model_name} network.")

    # momentum parameter is increased to 1. during training with a cosine schedule
    # High momentum (close to 1): Teacher changes very slowly, keeps most of its old weights
    if args.ema_mode == "epoch":
        """ Cosine Scheduler:
        A learning rate scheduling pattern that follows a cosine curve:
        - Starts high → gradually decreases → ends low
        """
    ...
    for epoch in range(start_epoch, args.pretrain_epochs):
        for i, data_loader in enumerate(data_loader_list_train): 
            # MEDMNIST MODIFICATION: Use dataset-specific loss function
            dataset_criterion = loss_functions[i]  # Use appropriate loss for each dataset
            train_one_epoch(model, i, dataset_list[i], data_loader, device, dataset_criterion, optimizer, epoch, args.ema_mode, teacher, momentum_schedule, coef_schedule, it)
            it += 1
        val_loss_list = []
        for i, dv in enumerate(data_loader_list_val):
            # MEDMNIST MODIFICATION: Use dataset-specific loss function for validation too
            dataset_criterion = loss_functions[i]  # Use appropriate loss for each dataset
            val_loss = evaluate(model, i, dv, device, dataset_criterion, dataset_list[i])
        ...
        if epoch % args.test_epoch == 0 or epoch+1 == args.pretrain_epochs:
            ...
            with open(output_file, 'a') as writer:
                ...
                for i, dataset in enumerate(dataset_list):
                    ...
                    # MEDMNIST MODIFICATION: Pass task type for proper activation function selection
                    task_type = datasets_config[dataset]['task_type']
                    y_test, p_test = test_classification(model, i, data_loader_list_test[i], device, multiclass, task_type)
                    y_test_teacher, p_test_teacher = test_classification(teacher, i, data_loader_list_test[i], device, multiclass, task_type)
                    if multiclass:
